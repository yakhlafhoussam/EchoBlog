<section class="w-[70%] h-full flex flex-col px-10 py-5 bg-slate-100">
    ...
    ?>
    <div class="w-full p-5 flex flex-wrap justify-between overflow-y-auto gap-y-4" style="-ms-overflow-style: none; scrollbar-width: none;">
        <?php 
        for ($h = count($_SESSION['blog']) - 1; $h >= 0; $h--) { 
            echo '
            <div class="w-full rounded-xl bg-white border-2 border-blue-700 border-solid">
            ...
            <div class="w-full py-[10px] relative">
                ...
                <p class="text-gray-600 w-full font-semibold p-3">' . nl2br(htmlspecialchars($_SESSION['blog'][$h]['content'])) . '</p>
            </div>';
            if (isset($_SESSION['id'])) {
                echo '

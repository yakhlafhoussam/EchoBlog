<section class="w-[70%] h-full flex flex-col px-10 py-5 bg-slate-100">
    ...
    <div class="w-full flex gap-2">
        ...
        <div class="flex flex-col p-5 mb-5 gap-2 bg-slate-200 rounded-xl w-1/4">
            ...
            <h1 class="text-2xl text-gray-500 font-semibold"><?php if (isset($_SESSION['authorcat'])) {
                                                                    echo '
        ...
        <div class="w-[48%] h-[300px] rounded-2xl bg-white">
            ...
            <form method="POST" action="addarticle" class="w-full flex flex-col p-4 gap-2 bg-white rounded-b-xl overflow-y-auto">
                <div class="relative">
                    ...
                    <input id="title" name="title" maxlength="60" value="<?php if (isset($_SESSION['title'])) {
                                                                                echo $_SESSION['title'];
                                                                                unset($_SESSION['title']);
                                                                            } ?>" type="text" placeholder="Title" class="w-full pl-10 pr-3 py-2 text-sm font-bold border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                ...
                <div class="relative">
                    ...
                    <textarea id="content" name="content" maxlength="1300" value="<?php if (isset($_SESSION['content'])) {
                                                                                        echo $_SESSION['content'];
                                                                                        unset($_SESSION['content']);
                                                                                    } ?>" type="text" placeholder="Content" class="w-full pl-10 pr-3 py-2 text-lg border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent h-14 resize-none"></textarea>
                    <p id="maxc" class="text-gray-400 font-bold absolute right-2 bottom-[6px]">1300</p>
                </div>
                <input id="categoryone" name="categoryone" class="w-0 h-0 hidden" type="text" value="">
                <div id="categorySelect" class="w-full h-10 border border-solid border-slate-200 rounded-md flex items-center gap-2 px-4">
                    <h1 id="empty" class="w-full h-full flex justify-center items-center text-2xl text-gray-400 font-bold">You will see the added category here</h1>
                </div>
    ...
    <div class="w-full bg-white rounded-xl mt-4">
        <div class="w-full h-16 flex items-center bg-blue-100 rounded-t-xl">
            <h1 class="w-[30%] h-full flex gap-5 items-center px-4 text-xl font-semibold text-gray-500"><i class="fas fa-header"></i> Title</h1>
            <h1 class="w-1/5 h-full border-l border-solid border-blue-600 flex gap-5 items-center px-4 text-xl font-semibold text-gray-500"><i class="fas fa-tags"></i> Category</h1>
            <h1 class="w-1/5 h-full border-l border-solid border-blue-600 flex gap-5 items-center px-4 text-xl font-semibold text-gray-500"><i class="fas fa-heart"></i> Likes</h1>
            <h1 class="w-1/5 h-full border-l border-solid border-blue-600 flex gap-5 items-center px-4 text-xl font-semibold text-gray-500"><i class="fas fa-comment"></i> Comments</h1>
            <h1 class="w-[10%] h-full border-l border-solid border-blue-600 flex gap-5 items-center px-4 text-xl font-semibold text-gray-500"><i class="fas fa-trash"></i> Delete</h1>
        </div>
        <div class="w-full max-h-56 flex flex-col bg-white rounded-b-xl overflow-y-auto">
            <?php
            if (count($_SESSION['article']) > 0) {
                for ($i=0; $i < count($_SESSION['article']); $i++) { 
                    echo '
                    <div class="w-full flex border-b border-solid">
                <div class="w-[30%] flex items-center px-4 py-2">
                    <h1 class="text-lg font-bold">' . $_SESSION['article'][$i]['title'] . '</h1>
                </div>
                <div class="w-1/5 border-l border-solid border-blue-600 flex items-center px-4 py-2">
                    <div class="px-1 cursor-pointer">
                        <span class="role-badge inline-flex items-center px-2 py-0.5 rounded-full text-lg font-medium bg-opacity-10 bg-[' . $_SESSION['article'][$i]['color'] . '] text-[' . $_SESSION['article'][$i]['color'] . ']">
                            <i class="fas fa-' . $_SESSION['article'][$i]['icon'] . ' mr-1 text-lg"></i> ' . ucfirst($_SESSION['article'][$i]['name']) . '
                        </span>
                    </div>
                </div>
                <div class="w-1/5 border-l border-solid border-blue-600 flex items-center px-4 py-2 gap-2">
                    <h1 class="text-2xl text-gray-500 font-semibold">' . $_SESSION['article'][$i]['likes_count'] . ' likes</h1>
                </div>
                <div class="w-1/5 border-l border-solid border-blue-600 flex items-center px-4 py-2 gap-2">
                    <h1 class="text-2xl text-gray-500 font-semibold">' . $_SESSION['article'][$i]['comments_count'] . ' comments</h1>
                </div>
                <div class="w-[10%] border-l border-solid border-blue-600 flex items-center px-4 py-2 gap-2">
                    <form method="POST" action="delarticle">
                        <input type="hidden" name="articledell" value="' . $_SESSION['article'][$i]['id'] . '">
